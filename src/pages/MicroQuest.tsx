...
const MicroQuest = () => {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => { playWhoosh(); setPhase("burst"); }, 3500);
    return () => { clearInterval(interval); clearTimeout(timeout); };
  }, [phase, captureMode]);

  useEffect(() => {
    if (phase !== "capture" || captureMode !== "image") return;
    const timeout = setTimeout(() => { playWhoosh(); setPhase("burst"); }, 2500);
    ...
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden px-6">
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-md w-full">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="w-full flex items-center justify-between mb-6">
          ...
          <button onClick={() => { playClick(); navigate("/dashboard"); }} className="text-muted-foreground hover:text-foreground aegis-transition">
            <X className="w-5 h-5" />
          </button>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          {phase === "select" && (
            <motion.div key="select" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col items-center w-full">
              ...
              <ProcessingButton variant="primary" onClick={handleCapture} className="w-full max-w-xs">
                BEGIN CAPTURE
              </ProcessingButton>
            </motion.div>
          )}
                ...
                <>
                  ...
                  <PremiumCard variant="outlined" className="w-64 mb-8" hoverable={false}>
                    <div className="relative h-40 flex items-center justify-center">
                      ...
                      <motion.div className="absolute inset-0 bg-primary/5" animate={{ opacity: [0, 0.1, 0] }} transition={{ duration: 2, repeat: Infinity }} />
                    </div>
                  </PremiumCard>
